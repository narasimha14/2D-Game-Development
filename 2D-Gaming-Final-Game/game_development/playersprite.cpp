...
bool PlayerSprite::getVisibility() const{
  return visible;
}
...
/* Required during the fight scene to limit the reach of the player so that he doesn't escape*/
void PlayerSprite::setStartOffset(unsigned int val){
  startOffset = val;
}
...
void PlayerSprite::update(Uint32 ticks) { 
  ...
  if(jump){
    /* If the player is in downward motion, increment its y position*/
    if(isFalling){
      ++jumpedHeight;
    ...
    /* If the player is in upward motion, decrement its y position*/
    else{
      ++jumpedHeight;
      if( jumpedHeight < maxJumpHeight/5){
        setPosition( getPosition() - Vector2f(0,5));
